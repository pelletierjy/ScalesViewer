...
When('the scale is applied to the keyboard', async function () {
  ...
  world.storeValue('currentHighlightedNotes', highlightedNotes);
});
...
When('the scale is displayed', async function () {
  ...
  world.storeValue('scaleNoteCount', scaleNoteCount);
});
...
Then('the highlighted notes should be C, D, E, F, G, A, B', async function () {
  ...
  for (const note of expectedNotes) {
    expect(highlightedNotes.some(n => n === note)).toBe(true);
  }
});
...
Then('the root note should have a different color or style than other scale notes', async function () {
  ...
  const rootNoteElements = world.page.locator(`text >> "${world.config.rootNote}"`);
  const count = await rootNoteElements.count();
  const expectedCount = world.config.octaveCount;
...
Then('I should see the correct scale pattern highlighted', async function () {
  const scaleName = world.config.scale;
  const rootNote = world.config.rootNote;
  
  // Get expected notes for this scale
  let expectedNotes: string[] = [];
  
  if (scaleName === 'Major' && scaleTestData.majorScaleNotes[rootNote]) {
    expectedNotes = scaleTestData.majorScaleNotes[rootNote];
  } else if (scaleName === 'Minor' && scaleTestData.minorScaleNotes[rootNote]) {
    expectedNotes = scaleTestData.minorScaleNotes[rootNote];
  } else if (scaleName === 'Pentatonic Major' && scaleTestData.pentatonicMajorNotes[rootNote]) {
    expectedNotes = scaleTestData.pentatonicMajorNotes[rootNote];
  } else if (scaleName === 'Pentatonic Minor' && scaleTestData.pentatonicMinorNotes[rootNote]) {
    expectedNotes = scaleTestData.pentatonicMinorNotes[rootNote];
  } else if (scaleName === 'Blues' && scaleTestData.bluesScaleNotes[rootNote]) {
    expectedNotes = scaleTestData.bluesScaleNotes[rootNote];
  } else {
...
Given('the display setting is set to show note names', async function () {
  ...
  world.config.showDegrees = false;
});
...
Then('the note names should be clearly readable', async function () {
  const scaleNoteElements = world.page.locator('circle + text');
...
Then('the text should contrast well with the key background', async function () {
  const textElements = world.page.locator('circle + text');
...
Then('each highlighted key should show its scale degree number', async function () {
  ...
  const hasScaleDegrees = highlightedNotes.some(note => /^[1-7]$/.test(note));
...
Then('each octave should show the same relative scale pattern', async function () {
  ...
  const highlightedNotes = await world.pianoPage.getHighlightedNotes();
  const octaveCount = world.config.octaveCount;
...
Then('the root note \\(C) should have a distinct color', async function () {
  ...
  const rootNoteElements = world.page.locator(`text >> "C"`);
  const count = await rootNoteElements.count();
  expect(count).toBeGreaterThan(0);
});

Then('the dominant note \\(G) should have a recognizable color', async function () {
  if (world.config.scale === 'Major' && world.config.rootNote === 'C') {
    const dominantNoteElements = world.page.locator(`text >> "G"`);
...
Then('other scale degrees should follow the established color scheme', async function () {
  const scaleNoteElements = world.page.locator('circle[fill]');
  const count = await scaleNoteElements.count();
  expect(count).toBeGreaterThan(0);
...
Then('the keyboard should remain fully interactive', async function () {
  ...
  await world.pianoPage.clickPianoKey(0);
  // Should not throw errors
});

Then('no error messages should be displayed', async function () {
  const errorElements = world.page.locator('[role="alert"], .error, .error-message');
...
Then('the previous scale highlights should be cleared', async function () {
  ...
  await world.waitForStabilization();
  const newHighlights = await world.pianoPage.getHighlightedNotes();
...
Then('the root note emphasis should update to {string}', async function (newRootNote: string) {
  ...
  if (world.config.rootNote !== newRootNote) {
    const isEmphasized = await world.pianoPage.verifyRootNoteEmphasis(world.config.rootNote);
    expect(isEmphasized).toBe(true);
  }
});
...
Then('all 12 notes should be highlighted', async function () {
  if (world.config.scale === 'Chromatic') {
    const scaleNoteCount = await world.pianoPage.getScaleNoteCount();
    const octaveCount = world.config.octaveCount;
...
Then('both white and black keys should be highlighted', async function () {
  const whiteKeyCount = (await world.pianoPage.getVisibleKeyCount()).white;
  const blackKeyCount = (await world.pianoPage.getVisibleKeyCount()).black;
  const scaleNoteCount = await world.pianoPage.getScaleNoteCount();
