...
Given('audio is enabled in the browser', async function () {
  await (this as any).world.pianoPage.mockAudioContext();
  ((this as any).world).audio.mockEnabled = true;
});
...
When('I click on the middle C key', async function () {
  await (this as any).world.pianoPage.clickNoteByName('C');
  ((this as any).world).trackAudioPlayback('C');
  await (this as any).world.waitForStabilization(200);
});

When('I click on the {word} key', async function (noteName: string) {
  await (this as any).world.pianoPage.clickNoteByName(noteName);
  ((this as any).world).trackAudioPlayback(noteName);
  await (this as any).world.waitForStabilization(200);
});

Then('I should hear the {word} note played', async function (expectedNote: string) {
  const lastPlayedNote = ((this as any).world).getLastPlayedNote();
...
Then('the key should provide visual feedback when clicked', async function () {
  ...
  const firstKey = (await (this as any).world.pianoPage.getAllKeys())[0];
  if (firstKey) {
    // Click and verify no errors occur
    await firstKey.click();
    await (this as any).world.waitForStabilization(100);
  }
...
Then('the audio should play immediately without delay', async function () {
  ...
  const audioHistory = ((this as any).world).audio.playbackHistory;
...
When('I click on the E key', async function () {
  await (this as any).world.pianoPage.clickNoteByName('E');
  ((this as any).world).trackAudioPlayback('E');
});

When('I click on the G key', async function () {
  await (this as any).world.pianoPage.clickNoteByName('G');
  ((this as any).world).trackAudioPlayback('G');
});

Then('I should hear a C note', async function () {
  const playbackHistory = ((this as any).world).audio.playbackHistory;
...
When('I click and hold a piano key', async function () {
  ...
  if (keys.length > 0) {
    ...
    await (this as any).world.page.mouse.down();
    ((this as any).world).storeValue('keyPressed', true);
  }
});
...
Then('the key should show a pressed state visually', async function () {
  ...
  expect(((this as any).world).getValue('keyPressed')).toBe(true);
});
...
When('I release the key', async function () {
  await (this as any).world.page.mouse.up();
  ((this as any).world).storeValue('keyPressed', false);
});

Then('the key should return to its normal visual state', async function () {
  expect(((this as any).world).getValue('keyPressed')).toBe(false);
});
...
When('I rapidly click multiple different keys in sequence', async function () {
  ...
  for (let i = 0; i < keysToClick; i++) {
    await keys[i].click();
    ((this as any).world).trackAudioPlayback(`key-${i}`);
    await (this as any).world.waitForStabilization(50); // Short delay between clicks
  }
});
...
When('I tap a piano key with my finger', async function () {
  ...
  if (keys.length > 0) {
    // Simulate touch tap
    await keys[0].tap();
    ((this as any).world).trackAudioPlayback('touch-note');
  }
});
...
Given('I am using keyboard navigation', async function () {
  ...
  await (this as any).world.page.focus('body');
});
...
When('I press the Right Arrow key', async function () {
  await (this as any).world.page.keyboard.press('ArrowRight');
  await (this as any).world.waitForStabilization(200);
});
...
When('I press the Space key or Enter', async function () {
  await (this as any).world.page.keyboard.press('Space');
  ((this as any).world).trackAudioPlayback('keyboard-activated');
  await (this as any).world.waitForStabilization(200);
});
...
When('I click on highlighted scale notes', async function () {
  ...
  if (highlightedNotes.length > 0) {
    await (this as any).world.pianoPage.clickNoteByName(highlightedNotes[0]);
    ((this as any).world).trackAudioPlayback('scale-note');
  }
});
...
When('I click on non-highlighted notes', async function () {
  ...
  if (keys.length > 0) {
    await keys[0].click();
    ((this as any).world).trackAudioPlayback('non-scale-note');
  }
});
...
When('I click on C in the first octave', async function () {
  // Find first C note
  const cNotes = ((this as any).world).page.locator('text >> "C"');
  if (await cNotes.count() > 0) {
    await cNotes.first().click();
    ((this as any).world).trackAudioPlayback('C-low');
  }
});
...
When('I click on C in the second octave', async function () {
  ...
  if (count > 1) {
    ...
    ((this as any).world).trackAudioPlayback('C-high');
  }
});
...
Given('I have audio settings configured', async function () {
  ...
  expect(((this as any).world).audio.mockEnabled).toBe(true);
});
...
When('I click a piano key', async function () {
  ...
  if (keys.length > 0) {
    await keys[0].click();
    ((this as any).world).trackAudioPlayback('configured-note');
  }
});
...
Given('audio is not available or blocked', async function () {
  ...
  ((this as any).world).audio.mockEnabled = false;
  await (this as any).world.page.addInitScript(() => {
...
Then('no error messages should disrupt the user experience', async function () {
  const errorElements = ((this as any).world).page.locator('[role="alert"], .error');
...
When('I rapidly click many keys in quick succession', async function () {
  ...
  for (let i = 0; i < clickCount; i++) {
    ...
    ((this as any).world).trackAudioPlayback(`rapid-${i}`);
...
When('I click multiple keys at exactly the same time', async function () {
  ...
  for (let i = 0; i < simultaneousKeys; i++) {
    ...
    ((this as any).world).trackAudioPlayback(`simultaneous-${i}`);
  }
...
Then('all clicked keys should show pressed state', async function () {
  ...
  const playbackHistory = ((this as any).world).audio.playbackHistory;
  const simultaneousCount = playbackHistory.filter((note: string) => note.startsWith('simultaneous-')).length;
...
Then('the screen reader should announce the key name and note', async function () {
  ...
  const focusedElement = ((this as any).world).page.locator(':focus');
  const ariaLabel = await (this as any).world.pianoPage.getAriaLabel(focusedElement);
...
When('I activate the key using assistive technology', async function () {
  await (this as any).world.page.keyboard.press('Enter');
  ((this as any).world).trackAudioPlayback('assistive-activation');
});
...
When('I simultaneously click C, E, and G keys', async function () {
  ...
  const clickPromises = notes.map(async (note) => {
    await (this as any).world.pianoPage.clickNoteByName(note);
    ((this as any).world).trackAudioPlayback(`chord-${note}`);
  });
...
Then('all three notes should play harmoniously together', async function () {
  const playbackHistory = ((this as any).world).audio.playbackHistory;
  const chordCount = playbackHistory.filter((note: string) => note.startsWith('chord-')).length;
...
When('I click and immediately release a piano key', async function () {
  ...
  if (keys.length > 0) {
    await keys[0].click();
    ((this as any).world).trackAudioPlayback('sustain-note');
...
When('I click multiple keys in quick succession', async function () {
  ...
  for (let i = 0; i < keysToClick; i++) {
    await keys[i].click();
    ((this as any).world).trackAudioPlayback(`succession-${i}`);
    await (this as any).world.waitForStabilization(100);
  }
});
...
Then('the notes should overlap naturally like a real piano', async function () {
  const playbackHistory = ((this as any).world).audio.playbackHistory;
  const successionCount = playbackHistory.filter((note: string) => note.startsWith('succession-')).length;
